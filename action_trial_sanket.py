...
class ActionDefaultFallback(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        try:
          word_digits=w2n.word_to_num(last_message)
        except:
          word_digits=[]
        dispatcher.utter_message("word_digits:{}".format(word_digits))
        numbers= digits + word_digits
        dispatcher.utter_message("numbers:{}".format(numbers))
        #full=tracker.latest_message['entities'][0]['value']
        #dispatcher.utter_message(full)
        #number=[]
        #cardinal=next(tracker.get_latest_entity_values('CARDINAL'), None)
        #cardinal=[w2n.word_to_num(i) for i in tracker.get_latest_entity_values('CARDINAL')]
        """
        cardinal=[i for i in tracker.get_latest_entity_values('CARDINAL')]
        dispatcher.utter_message("cardinal:{}".format(cardinal))
        #for i in len(cardinal):
        # dispatcher.utter_message("cardinal:{}".format(i))
        date=[i for i in tracker.get_latest_entity_values('DATE')]
        dispatcher.utter_message("date:{}".format(date))
        numbers= cardinal + date
        dispatcher.utter_message("numbers:{}".format(numbers))
        """
        #interview starter
        ...
        #the slot interview_state tells us if the intervew started or not. we will be writing such
        # if conditions as below for interview state= "start" also. that will be if the person is
        # not in the interview and talking to the bot.
        
        #after interview start
        ## intents which require intervention and question repeat!
        elif interview_state=="started":
          ...
          ## currents where customer answer dicatets a bifurcation!
