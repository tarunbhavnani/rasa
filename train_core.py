...
def train_dialogue(domain_file, model_path, training_folder):
    ...
    agent = Agent(domain_file, policies=[
            MemoizationPolicy(max_history=12),
            #KerasPolicy(MaxHistoryTrackerFeaturizer(LabelTokenizerSingleStateFeaturizer(),max_history=20),epochs=300), fallback])
            KerasPolicy(MaxHistoryTrackerFeaturizer(BinarySingleStateFeaturizer(),max_history=5),epochs=300), fallback])
